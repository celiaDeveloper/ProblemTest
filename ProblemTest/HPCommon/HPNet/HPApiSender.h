...
/** 发送相关接口参数 */
typedef void(^sendBlock)(HPRequestParameter *request);
...
/*! 请求失败的 block */
typedef void( ^failureBlock)(HPReconnectView *connectview,NSError *error);

@interface HPApiSender : NSObject

/** 相当于变量,重新连接用此变量,非重新连接重新创建对象 */
@property(nonatomic,strong)HPRequestParameter *request;
...
/**
 ...
 @param failure 请求失败的block(return NSError),一般接口错误,服务器错误,网络问题
 */
