...
@interface HPRequestParameter()

@end

@implementation HPRequestParameter
...
-(void)setStatus:(HPRequestResultStatus)status
{
    ...
    switch (status) {
        ...
        {
            //因为每次重新连接，HPRequestParameter这个都会重新创建，self.reconnectView也会跟着变化，如果用self.reconnectView这个去判断是否已经是self.contentView的子视图，结果一直是否，会导致self.contentView这个视图一直叠加重新连接的视图。
