...
#import "HPChineseSort.h"
//特殊字符统一合并后的  显示字符
#define AbnormalLetter @"#"

@interface HPChineseSort()
...
+(NSMutableArray*)IndexArray:(NSArray*)stringArr{
    ...
    {
        NSString *pinyin = [((HPChineseSort*)object).pinYin substringToIndex:1];
        //不同
...
+(NSMutableArray*)LetterSortArray:(NSArray*)stringArr{
    ...
    for (NSString* object in tempArray) {
        ...
        NSString *pinyin = [((HPChineseSort*)object).pinYin substringToIndex:1];
        NSString *string = ((HPChineseSort*)object).string;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr{
    ...
    {
        HPChineseSort *chineseString = [[HPChineseSort alloc]init];
        ...
        chineseString.string = [HPChineseSort RemoveSpecialCharacter:chineseString.string];
...
//    CFAbsoluteTime startTime = CFAbsoluteTimeGetCurrent();
    ...
    NSString *tempString;
    //拼音分组
    for (HPChineseSort* object in tempArray) {
